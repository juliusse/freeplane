...
@echo on
java -Xmx512m "-Dorg.knopflerfish.framework.bundlestorage=memory" "-Dorg.freeplane.main.application.FreeplaneStarter.headless=true" "-Dorg.knopflerfish.gosg.jars=reference:file:%freeplanedir%core/" %resdir% %defines% -jar "%freeplanedir%framework.jar" -xargs "%freeplanedir%props.xargs" -xargs "%freeplanedir%%xargs%"
